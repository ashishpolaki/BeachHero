...
namespace BeachHero
{
    public class Boat : MonoBehaviour
    {
        ...
        private int STATIC_IDLE_HASH = Animator.StringToHash(StringUtils.STATIC_IDLE_ANIM);
        ...
        private void PlayStaticIdleAnimation()
        {
            if (characterAnimator == null) return;
            characterAnimator.Play(STATIC_IDLE_HASH, -1, Random.Range(0f, 1f));
        }
        ...
        public void SetBoatInCustomisationScreen(int currentBoatIndex, int currentColorIndex)
        {
            PlayStaticIdleAnimation();
            for (int m = 0; m < boatMeshRenderer.Length; m++)
            {
                boatMaterial = boatMeshRenderer[m].material;
                boatMaterial.SetFloat(Shader.PropertyToID(StringUtils.BOAT_REPLACEABLE_COLORS_KEY), 1f);
